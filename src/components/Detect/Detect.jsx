...
const Detect = () => {
  ...
  // Auto speak whenever gestureOutput changes
  useEffect(() => {
    if (gestureOutput && gestureOutput.trim() !== "") {
  ...
  }, [gestureOutput, selectedVoice, rate, pitch]);

  // Manual control for speaking/stopping
  const handleSpeak = () => {
    if (!gestureOutput || gestureOutput.trim() === "") return;
    if (isSpeaking) {
        ...
        {accessToken ? (
          <>
            ...
            <div className="tts-container">
              <h2 className="gradient__text">Speech Output</h2>
              {/* <label>
                Voice:
                <select
